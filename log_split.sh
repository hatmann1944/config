...
log_dir=/srv/space/data/sgateway/logs
pid_file=/srv/space/data/sgateway/logs/nginx.pid
...
do
    ...
    if [ -f "$file" -a ! -e "$log_dir/${bname}.${last_hour}" ]; then
        mv $log_dir/${bname} ${bname}.${last_hour}
    fi
    ...
    ln -s -f $log_dir/$bname.$cur_hour $log_dir/${bname}
done
...
for oldfiles in $(find $log_dir -mtime +7 -iname '*.log*')
do
    echo $oldfiles
    rm -rf $oldfiles
done
